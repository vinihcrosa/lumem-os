...
beforeEach(async () => {
  ...
  database = openTestDb();
  const config = loadConfig({ LUMEM_STATE_DIR: tempDir("lumem-state-") });
...
async function postBatched(path: string, input: unknown): Promise<LightMyRequestResponse> {
  return app.inject({ authority,  method: "POST", url: `/trpc/${path}?batch=1`, payload: { "0": input } });
}
...
async function setupProject(): Promise<{ projectId: string; repo: string }> {
  ...
  const workspace = await app.inject({ authority, 
    method: "POST",
    url: "/trpc/workspace.create",
    payload: { name: "pessoal" },
  });
  const project = await app.inject({ authority, 
    method: "POST",
...
describe("files over http", () => {
  it("carries a write at the byte ceiling instead of answering 413", async () => {
    ...
    const read = await app.inject({ authority, 
      method: "GET",
  ...
  it("answers deletePreview over GET, and answers no write over GET at all", async () => {
    ...
    const preview = await app.inject({ authority, 
      method: "GET",
    ...
    for (const path of ["files.write", "files.create", "files.rename", "files.remove"]) {
      const response = await app.inject({ authority,  method: "GET", url: queryUrl(path, input) });
