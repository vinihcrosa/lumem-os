...
CREATE TABLE `__new_agent_config` (
	...
	CONSTRAINT "agent_config_adapter_version" CHECK("__new_agent_config"."retired_at" IS NOT NULL OR "__new_agent_config"."adapter_version" IS NOT NULL)
);
--> statement-breakpoint
INSERT INTO `__new_agent_config`("id", "name", "command", "args", "env", "adapter_version", "retired_at", "created_at", "updated_at") SELECT "id", "name", "command", "args", "env", "adapter_version", CASE WHEN "transport" = 'pty' THEN (unixepoch('subsec') * 1000) END, "created_at", "updated_at" FROM `agent_config`;--> statement-breakpoint
...
--> statement-breakpoint
INSERT INTO `__new_session`("id", "kind", "script_name", "agent_config_id", "scope_type", "scope_id", "cwd", "command", "state", "exit_code", "transport", "acp_session_id", "mode", "model", "lumem_mode", "resumed_from_id", "task_id", "task_role", "pending_prompt", "pending_reason", "created_at", "updated_at") SELECT "id", "kind", "script_name", "agent_config_id", "scope_type", "scope_id", "cwd", "command", "state", "exit_code", "transport", "acp_session_id", "mode", "model", "lumem_mode", "resumed_from_id", "task_id", "task_role", NULL, NULL, "created_at", "updated_at" FROM `session`;--> statement-breakpoint
