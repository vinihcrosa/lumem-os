...
-- Quinta vez, e a lição já tem endereço no `testing.md`: o `drizzle-kit` gera o
-- SELECT com as colunas da tabela nova lendo da velha. Aqui as duas que não
-- existem ainda são `prepared_prompt` e `prepared_role`; as duas ficam `NULL`,
-- que quer dizer *nada preparado* — a verdade sobre toda tarefa anterior ao
-- degrau `assistido`.
INSERT INTO `__new_task`("id", "workspace_id", "project_id", "title", "body", "status", "created_by", "created_by_session", "worktree_id", "position", "links", "reason", "attempts", "autonomy", "status_changed_at", "closed_at", "created_at", "updated_at") SELECT "id", "workspace_id", "project_id", "title", "body", "status", "created_by", "created_by_session", "worktree_id", "position", "links", "reason", "attempts", "autonomy", "status_changed_at", "closed_at", "created_at", "updated_at" FROM `task`;--> statement-breakpoint
