...
async function cloneFrom(page: Page, source: string, name: string): Promise<void> {
  ...
  await dialog.getByLabel("Caminho ou URL").fill(source);
  await dialog.getByLabel("Nome").fill(name);
  await dialog.getByRole("button", { name: "clonar" }).click();
  // Q5: o diálogo NÃO fecha aqui — ele vira o progresso do clone, e só fecha
  // quando o clone termina bem. Esperar por isso é esperar pelo clone.
...
test("recusa ext:: e git:// na tela, antes de qualquer processo", async ({ page }) => {
  ...
  const dialog = page.getByRole("dialog");
  await dialog.getByLabel("Caminho ou URL").fill("ext::sh -c id");
  ...
  await dialog.getByLabel("Caminho ou URL").fill("git://host/repo.git");
...
test("clona um repositório vazio, e a tela explica por que ele ainda não corta worktree", async ({
  ...
  // O `+` da linha abre; quem explica é o diálogo. Desabilitá-lo seria um botão
  // de 24px cinza numa linha de árvore, com o motivo fora da tela.
  await page.getByRole("button", { name: "nova worktree em dia-zero" }).click();
...
test("remover um projeto registrado por caminho não apaga o repositório dele", async ({ page }) => {
  ...
  await openDialog(page);
  const dialog = page.getByRole("dialog");
  await dialog.getByLabel("Caminho ou URL").fill(E2E_FIXTURE_REPO_ORIGIN);
  await dialog.getByLabel("Nome").fill("apontado");
