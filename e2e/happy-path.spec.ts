...
test("the whole flow, from an empty install to a removed worktree", async ({ page, request }) => {
  ...
  // Both alive at once: the worktree shell is still listed and still running.
  const worktreeSessions = page.getByLabel(/^sessões de /).first();
  await expect(worktreeSessions).toBeVisible();
  ...
  await page.locator('aside li[data-kind="agent"][data-scope="project"] > button').last().click();
...
async function closeEverySession(page: Page): Promise<void> {
  const running = page.locator('li[data-state="running"] > button');
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    ...
    // A session can end on its own between being listed and being clicked —
    // the close button simply is not there for one that already stopped.
