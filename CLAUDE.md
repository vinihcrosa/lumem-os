...
Cinco features de pé — [walking-skeleton](docs/prd/walking-skeleton/tasks.md), [ui-shell](docs/prd/ui-shell/tasks.md), [worktree-tabs](docs/prd/worktree-tabs/tasks.md), [right-panel](docs/prd/right-panel/tasks.md) e [file-editor](docs/prd/file-editor/tasks.md) — esta última faz o daemon **escrever** no repositório, com autosave e CRUD pela árvore. Em pilha de PRs, com 01 a 04, S1 e S2 já mergeadas e a 05 no topo: [workspace-memory](docs/prd/workspace-memory/tasks.md), o self-learning com memória compartilhada de workspace — o `~/.lumem` versionado pelo daemon, os sinais de ação, o portão de escrita, as superfícies (CLI + router tRPC, shadow entre escopos, funil de acesso desligado), o recall lexical explicável e a **inbox de propostas na tela** — o [tasks.md](docs/prd/workspace-memory/tasks.md) diz onde cada parte parou. **Decidido em 2026-08-17:** a sessão de agente migra de PTY para [ACP](docs/project/pty-vs-acp.md) — a próxima feature a desenhar é `acp-sessions` (transporte + tela da conversa); o PTY fica para shell e como caminho alternativo. Comece pelo [índice da documentação](docs/README.md).
