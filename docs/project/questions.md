...
> [features/007-workspace-memory/prd.md](../features/007-workspace-memory/prd.md) e as perguntas refinadas, com proposta
> pra reagir em cada uma, em [open-questions.md](../features/007-workspace-memory/open-questions.md). As
...
**R:** respondida pela feature — [§3 do PRD](../features/007-workspace-memory/prd.md): três naturezas, **fato**, **procedimento** e **contrato**, com taxonomia fechada de 7 tipos na [PR 02](../features/007-workspace-memory/tasks.md). Não é um balde só chamado "memória".
...
**R:** respondida pela feature — **(c) híbrido**, e é a assimetria que sustenta o desenho ([§1 do PRD](../features/007-workspace-memory/prd.md)): o que é barato de errar (projeto) pode ser automático; o que é caro de errar (workspace) entra como **proposta** na inbox e passa por você ([Q27](../features/007-workspace-memory/open-questions.md): `domain`, `process` e `contract` escritos por agente viram proposta; `project` e `reference` vão direto).
...
**R:** respondida pela feature — **(b) markdown fora do repo**, em `~/.lumem/`, com o **banco derivado e reconstruível** por `reindex` (premissas A1–A4 de [tasks.md](../features/007-workspace-memory/tasks.md)). E o `~/.lumem` é **versionado por git pelo próprio Lumem** ([Q36](../features/007-workspace-memory/open-questions.md)), o que devolve o histórico sem devolver o lixo ao repositório. Só o `id` do projeto fica dentro do repo, em `.lumem/project.toml` ([Q3.1](../features/007-workspace-memory/open-questions.md)).
...
**R:** respondida pela feature — **precedência fixa por especificidade, e shadow em vez de merge** ([§7 do PRD](../features/007-workspace-memory/prd.md)): projeto > workspace > global, identidade `(tipo, slug)`. O perdedor continua no disco e o sombreamento vira evento. Nada é concatenado em silêncio.
...
A [pull-request-status](../features/013-pull-request-status/prd.md) escreveu o `PrHost`: *"dado um repositório
...
numa revisão de código.** A [pull-request-status](../features/013-pull-request-status/prd.md) trouxe
...
> por remote ([Q3.1](../features/007-workspace-memory/open-questions.md)). Fica `[ ]` aqui porque a decisão do
