...
| `server/` **quem fala com o daemon** — `Host`, `Origin`, `Sec-Fetch-Site` | três camadas: unit pura sobre um registro de requisição (`origin-policy.test.ts`); integration por `app.inject` com os cabeçalhos de cada ataque; e integration com **listener de verdade e cliente `ws` cru**, porque a afirmação é sobre o que acontece **antes** do handshake — uma recusa tem que chegar como status HTTP em `unexpected-response`, nunca como `open`. Consequência para a suíte inteira: todo `app.inject` de transporte manda `authority`, porque o `localhost:80` default do `light-my-request` é, para o guarda, uma página que fez rebinding. **Mutação exigida pelo PRD e conferida:** o roteador ignorando o guarda derruba 2 testes; o `createServer` sem registrá-lo derruba 9 | Sim |
