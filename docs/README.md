...
| [tasks.md](features/028-autonomous-orchestration/tasks.md) | **12 tasks em 5 fases, e só a F1.** O §0 registra o corte e o motivo. As fases **0 e 1 estão entregues** — o desenho sincronizado, o estudo escrito, os três estados novos no modelo com a migração conferida na recriação de tabela, e a `position` que faz a posição na coluna ser a prioridade. Duas tasks nascem de contradição achada lendo código entregue: a [T3](features/028-autonomous-orchestration/tasks.md) (faltam dois estados, e `Backlog`/`To-Do` colapsariam na fronteira de autorização) e a [T4](features/028-autonomous-orchestration/tasks.md), cujos cinco casos **nasceram todos verdes**: o arrasto para qualquer coluna já funcionava, e **nada o cobria** — era verdadeiro por acidente |
