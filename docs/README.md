...
| [orchestration-measurements.md](project/orchestration-measurements.md) | A **medição** que o §11 da [028](features/028-autonomous-orchestration/prd.md) pedia, feita em 2026-09-12: `end_turn` não distingue *"terminei"* de *"te perguntei"* — **4 de 13 turnos reais** significaram terminei —, o precedente do `gh` não é portável para tracker (não existe `linear` na máquina), polling cabe em **2,4% da cota** do Linear, e a camada gerenciada custa **~2 s** por chamada contra ~345 ms do caminho direto |
...
**Em execução desde 2026-09-12, e só com a F1** — o quadro lendo a `022`, com a autonomia desligada.
Seis das nove conversas do §11 continuam guardadas e duas não têm resposta, uma delas exigindo ADR
novo; a F1 não depende de nenhuma. A fase 0 está entregue, e o
...
| [tasks.md](features/028-autonomous-orchestration/tasks.md) | **12 tasks em 5 fases, e só a F1.** O §0 registra o corte e o motivo. A fase 0 está entregue — o desenho sincronizado e o estudo escrito. Duas tasks nascem de contradição achada lendo código entregue: a [T3](features/028-autonomous-orchestration/tasks.md) (faltam dois estados, e `Backlog`/`To-Do` colapsariam na fronteira de autorização) e a [T4](features/028-autonomous-orchestration/tasks.md) (`in_progress` é derivado e **nunca teve escritor humano**, então o "arraste para qualquer coluna, sempre" do §4 não era implementável — faltava o escritor, não a decisão) |
| [open-questions.md](features/028-autonomous-orchestration/open-questions.md) | **39, e 37 respondidas**, em seis rodadas. Cinco respostas vieram contra a proposta. A melhor pergunta é do Vinicius, dentro da Q21 — *"quando o cartão está em In Review, eu tenho garantia que um agente já pegou para revisar?"* — e ela mostrou que **três das seis colunas não sabiam dizer se alguém estava trabalhando nelas**. As Q34–Q37 não vieram de discussão: vieram de **medir a tela**, e três contradiziam a PRD. A sexta rodada é outra coisa ainda — as **Q38–Q40** vieram de **ler o código que já está de pé**. A Q38 é a única do documento que **já estava respondida** (pela Q3), e a proposta que veio com ela estava errada por confundir coluna com selo; o que sobrou dela é a **Q40** — com a esteira ligada, a fila pegaria o cartão que você arrastou para fazer na mão. A Q40 e a **Q39** (um estado do selo que **tem pixel desenhado e não tem fonte de dado**) são as duas abertas, ambas represadas até a F2 |
